...
const ToggleComponent: React.FC<ToggleComponentProps> = ({ text, checked, toggle }) => {
  return (
    <div
      className="flex flex-row gap-3 py-1 ml-2 cursor-pointer align-center"
      onClick={toggle}
      title={text}
    >
      <div className="grow text-xs text-gray-200 uppercase break-words">{text}</div>
      <div className="flex content-center flex-none">
        <div className="relative">
          <div className="w-10 h-4 bg-gray-600 rounded-full shadow-inner"></div>
          <div
            className={joinClassNames(
              'absolute w-6 h-6 transition rounded-full shadow -left-1 -top-1',
              checked
