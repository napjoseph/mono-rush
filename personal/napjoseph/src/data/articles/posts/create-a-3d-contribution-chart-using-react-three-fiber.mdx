...
- The `scaledHeight` function calculates `Box` heights based on their value, the maximum value in the dataset, and the `scale` factor. The final height is determined by dividing the cell value by the maximum value, and then multiplying by the `scale` factor. This ensures that each cell's height is proportional to its value compared to the maximum value in the data set.
...
> **NOTE**: For more information about the ordered triplet of numbers used to represent a position in 3D space, see the Three.js documentation about [`Vector3`](https://threejs.org/docs/#api/en/math/Vector3).
